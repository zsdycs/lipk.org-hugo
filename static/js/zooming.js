...
(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.Zooming = factory());
  ...
  var target = {
    ...
    grab: function grab(x, y, scaleExtra) {
      ...
      setStyle(this.el, {
        cursor: cursor.grab,
        transform: '\n        translate3d(' + this.translate.x + 'px, ' + this.translate.y + 'px, ' + TRANSLATE_Z + 'px)\n        scale(' + (this.scale.x + scaleExtra) + ', ' + (this.scale.y + scaleExtra) + ')'
      });
    },
    move: function move(x, y, scaleExtra) {
      setStyle(this.el, {
        transition: 'transform',
        transform: '\n        translate3d(' + (this.translate.x + (x - clientX)) + 'px, ' + (this.translate.y + (y - clientY)) + 'px, ' + TRANSLATE_Z + 'px)\n        scale(' + (this.scale.x + scaleExtra) + ',' + (this.scale.y + scaleExtra) + ')'
      });
    },
    ...
    calculateTranslate: function calculateTranslate() {
      ...
      var targetCenter = {
        ...
        x: Math.ceil(windowCenter.x - targetCenter.x),
        y: Math.ceil(windowCenter.y - targetCenter.y)
      };
    },
    calculateScale: function calculateScale() {
      ...
      if (!customSize && zoomingHeight && zoomingWidth) {
        return {
          x: Math.ceil(zoomingWidth / this.rect.width),
          y: Math.ceil(zoomingHeight / this.rect.height)
        };
      } else if (customSize && (typeof customSize === 'undefined' ? 'undefined' : _typeof(customSize)) === 'object') {
        return {
          x: Math.ceil(customSize.width / this.rect.width),
          y: Math.ceil(customSize.height / this.rect.height)
        };
      } else {
        ...
        if (customSize && typeof customSize === 'string') {
          ...
          if (scale > maxZoomingWidth || scale > maxZoomingHeight) {
            return {
              x: Math.ceil(maxZoomingWidth),
              y: Math.ceil(maxZoomingHeight)
            };
          }
        }
  ...
  var Zooming = function () {
    ...
    createClass(Zooming, [{
      ...
      value: function move(x, y) {
        ...
        this.released = false;
        this.body.style.cursor = cursor.grab;
